...
class AscendCompressorStateCache(CompressorStateCache):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        ...
        return AscendSlidingWindowMLASpec(
            ...
            # The V2 runner uses this marker to preserve the physical DSA
            # cache layout instead of applying the generic MLA cache path.
...
class DeepseekV4Model(nn.Module, EagleModelMixin):
    ...
    def forward(
        ...
        # Hash-based MoE routing uses token IDs. In the V2 runner Ascend
        # extensions live in ForwardContext.additional_kwargs, so retain them
        # through the unified proxy rather than adding a V1-only attribute.
