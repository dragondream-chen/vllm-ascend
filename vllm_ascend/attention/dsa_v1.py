...
class AscendDSAImpl(DSAAttentionImpl):
    ...
    def forward(  # type: ignore[override]
        ...
        # Ascend stores extension fields in ForwardContext.additional_kwargs
        # under the v2 runner. _EXTRA_CTX preserves the direct-attribute
        # access used by v1 while reading that v2 storage correctly.
        num_tokens = _EXTRA_CTX.num_tokens
        assert num_tokens is not None, "Ascend forward context must provide num_tokens."
        o_proj_input_shape = (num_tokens, self.n_local_heads, self.head_dim)
